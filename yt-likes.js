(function() {
    ...
    const injectReviews = () => {
        ...
        // --- 2. أسماء وعوائل جديدة كلياً (من الجنوب لم تُستخدم سابقاً) ---
        const mFirst = ["محسن", "هادي", "طامي", "متعب", "نايف", "علي", "يحيى", "قاسم", "إبراهيم", "جابر"];
        const fFirst = ["صالحة", "فاطمة", "أريج", "ليلى", "نوال", "عائشة", "منيرة", "سماح", "خلود", "سارة"];
        const lNames = ["المجرشي", "الشراحيلي", "الحكمي", "النعمي", "المجهلي", "الفيفي", "الغزواني", "التليدي", "المالكي", "الحريصي"];

        // --- 3. دالة توليد تعليق ذكي (مختصر وغير مبالغ فيه) ---
        const generateSmartComment = () => {
            const starts = ["متجر ثقة", "شغل ممتاز", "خدمة جيدة", "تجربة ناجحة", "يعطيكم العافية", "بكل أمانة", "أنصح بالتعامل", "سرعة ممتازة", "متجر بطل", "خدمة سريعة"];
            const middles = ["لايكات وصلو سريع سريعين", "مافي نقص واللايكات ثابتة", "أفضل متجر للايكات يوتيوب", "تنفيذ فوري وضمان حقيقي", "وصلت اللايكات بلمح البصر", "جودة الخدمة ممتازة", "انصحكم فيه وبقوة", "ما قصرتوا على السرعة", "لايكات حقيقية وسريعة", "شغل مرتب وسريع سريعين"];
            // إيموجيات جديدة جداً (⭐، 🎖️) ومحدودة جداً (فقط 3 احتمالات من أصل 10)
            const ends = ["", "", "", "", "", "⭐", "", "", "🎖️", ""]; 
        ...
        // --- 4. دالة توليد وقت (تبدأ من ساعة وفوق وغير مكررة) ---
        const getUniqueTime = () => {
            const timeOptions = ["منذ ساعتين", "منذ 4 ساعات", "منذ 9 ساعات", "منذ 15 ساعة", "منذ 23 ساعة", "منذ 3 أيام", "منذ 6 أيام"];
            return timeOptions[Math.floor(Math.random() * timeOptions.length)];
        };

        // --- 5. حقن التقييمات (15 نص و 11 نجوم) ---
        let reviewPool = [];
        for (let i = 0; i < totalReviews; i++) {
            reviewPool.push(i < 15 ? generateSmartComment() : "");
        }
