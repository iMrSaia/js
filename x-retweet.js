(function() {
    ...
    const injectReviews = () => {
        ...
        // --- 2. أسماء وعوائل جديدة كلياً (مختلفة عن كل الخدمات السابقة) ---
        const mFirst = ["جسار", "هتان", "تامر", "مجاهد", "نبيل", "كنان", "ساهر", "فادي", "أمين", "صفي"];
        const fFirst = ["ديم", "رسيل", "سلاف", "وجد", "ميار", "تولين", "ليان", "وسن", "ليال", "جنى"];
        const lNames = ["باعقيل", "المنصوري", "الصعيدي", "النفيسة", "السلمان", "الخضير", "الحيدر", "السويلم", "المجلي", "الدخيل"];

        // --- 3. دالة توليد تعليق ذكي (ريتويت تويتر - بدون تكرار) ---
        const generateSmartComment = () => {
            const starts = ["بكل أمانة المتجر", "تجربة ممتازة جدا", "الله يبارك لكم", "خدمة سريعة كالعادة", "افضل متجر ريتويت", "تعامل احترافي", "انصح وبقوة", "سرعة بالانجاز", "ما قصرتوا والله", "مصداقية تامة"];
            const middles = ["الريتويت وصل سريع سريعين", "التنفيذ فوري وما فيه نقص", "أفضل متجر لزيادة الريتويت", "الريتويت ثابت والضمان حقيقي", "سرعة خرافية في التنفيذ", "خدمة توب وجودة عالية", "انصح فيه للي يبي تفاعل سريع", "بثواني وصل الريتويت للحساب", "شغل مرتب ومضمون جدا", "التنفيذ آلي وسريع جدا"];
        ...
        // --- 4. دالة توليد وقت فريدة (غير مكررة عن الخدمات السابقة) ---
        const getUniqueTime = () => {
            const timeOptions = ["منذ 7 دقائق", "منذ 20 دقيقة", "منذ 45 دقيقة", "منذ 15 ساعة", "منذ 18 ساعة", "منذ 4 أيام"];
            return timeOptions[Math.floor(Math.random() * timeOptions.length)];
        };

        // --- 5. حقن التقييمات (30 نص و 7 نجوم) ---
        let reviewPool = [];
        for (let i = 0; i < totalReviews; i++) {
            reviewPool.push(i < 30 ? generateSmartComment() : "");
        }
